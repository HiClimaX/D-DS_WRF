...
awk -F, '(NR > 1) && $2' model_conf/"$source_id".csv |
	parallel -j 4 --delay 3 --lb --colsep=, python get_6h_url.py \
		...
		--source_id "$source_id" --member_id "$member_id" --experiment_id historical --start_date "2010-01-02" --end_date "2010-01-30" |
	tee "$data_dir/urls.txt"

awk -F, '(NR > 1) && $2' model_conf/"$source_id".csv |
	parallel -j 4 --delay 3 --lb --colsep=, python get_6h_url.py \
		"${options[@]}" --variable_id '{1}' --table_id '{2}' --frequency '{3}' \
		--source_id "$source_id" --member_id "$member_id" --experiment_id ssp370 --start_date "2050-01-02" --end_date "2050-01-30" |
